...
var config = {
	...
	exportUrl: 'vehicle-driver-export.do'
};
  ...
  <div class="panel-body">
      ...
      <div style="margin-bottom: 20px;">
	    <div class="pull-left btn-group" role="group">
		  <button class="btn btn-default a-insert" onclick="location.href='vehicle-driver-input.do'">新建</button>
...
<form id="vehicle-infoGridForm" name="vehicle-infoGridForm" method='post' action="vehicle-driver-remove.do" class="m-form-blank">
      <div class="panel panel-default">
        <div class="panel-heading">
		  <i class="glyphicon glyphicon-list"></i>
  ...
  <table id="vehicle-infoGrid" class="table table-hover">
    <thead>
      <tr>
        ...
        <th class="sorting" name="name">驾驶证号</th>
        <th class="sorting" name="name">证件到期时间</th>
        <th class="sorting" name="name">驾龄(年)</th>
        <th class="sorting" name="name">准驾车型</th>
        <th class="sorting" name="name">地址</th>
        <th class="sorting" name="name">联系电话</th>
        <th>&nbsp;</th>
      </tr>
    </thead>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>${item.gender}</td>
        <td>${item.code}</td>
        <td><fmt:formatDate value="${item.expireDate}" type="date"/></td>
        <td>${item.year}</td>
        <td>${item.type}</td>
        <td>${item.location}</td>
        <td>${item.mobile}</td>
...
<c:choose> 
  ...
  <c:when test="${item.status == 3}">   
    ...
    报废 
        ...
        <td>
          <a href="vehicle-driver-input.do?id=${item.id}" class="a-update"><spring:message code="core.list.edit" text="编辑"/></a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
