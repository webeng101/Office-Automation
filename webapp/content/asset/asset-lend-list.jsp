  ...
  <div class="panel-body">
      ...
      <div style="margin-bottom: 20px;">
	    <div class="pull-left btn-group" role="group">
		  <button class="btn btn-default a-insert" onclick="location.href='asset-lend-input.do'">新建</button>
  ...
  <table id="asset-lendGrid" class="table table-hover">
    <thead>
      <tr>
        ...
        <th class="sorting" name="name">领用人</th>
        <th class="sorting" name="name">领用时间</th>
        <th class="sorting" name="name">归还时间</th>
		<!--
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>${item.lendDate}</td>
        <td><fmt:formatDate value="${item.returnDate}" type="both"/></td>
		<!--
        <td>${item.operator}</td>
		-->
        <td>${item.status == '0' ? '借出' : '归还'}</td>
        <td>
          <a href="asset-lend-input.do?id=${item.id}" class="a-update"><spring:message code="core.list.edit" text="编辑"/></a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
