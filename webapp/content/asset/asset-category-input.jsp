...
<form id="asset-infoForm" method="post" action="asset-category-save.do" class="form-horizontal">
  ...
  <div class="form-group">
	...
	<div class="col-sm-5">
	  <select name="parentId" class="form-control">
	    ...
	    <option value="${item.id}" ${item.id==model.assetCategory.id ? 'selected' : ''}>${item.name}</option>
		</c:forEach>
	  </select>
    </div>
  </div>
